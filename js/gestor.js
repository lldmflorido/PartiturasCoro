...
function iniciarSistema() {
    ...
    fetch(url, { headers: { "Authorization": "token " + tokenActual } })
        .then(function(res) {
...
function solicitarEliminacion(index) {
    ...
    pedirConfirmacion('¿Está completamente seguro de que desea eliminar el canto "' + canto.nombre + '"? Esta acción borrará la entrada del índice (El PDF físico se mantendrá por seguridad).')
...
document.getElementById('btn-guardar-local').addEventListener('click', function() {
    ...
    promesaArchivo.then(function() {
        ...
        if (conflictoNombre && inArchivo.files.length > 0) {
            return pedirConfirmacion('Ya existe otro canto usando el archivo "' + nombreArchivoFisico + '". ¿Desea sobrescribirlo de todas formas?');
        }
...
btnSincronizar.addEventListener('click', function() {
    pedirConfirmacion('¿Está seguro de enviar ' + cambiosPendientes.length + ' modificaciones al servidor? Los cambios se verán reflejados en todos los dispositivos.')
        .then(function(confirmado) {
            if (!confirmado) return;

            var modalCarga = document.getElementById('modal-carga');
            var barraProgreso = document.getElementById('barra-progress'); // Fix: was barra-progreso in HTML
            if(!barraProgreso) barraProgreso = document.getElementById('barra-progreso');
            ...
            var headers = { "Authorization": "token " + tokenActual, "Content-Type": "application/json" };
            ...
            cambiosPendientes.forEach(function(cambio) {
                sequencia = sequencia.then(function() {
                    completados++;
                    textoCarga.textContent = 'Procesando: Elemento ' + completados + ' de ' + cambiosPendientes.length;
                    
                    if ((cambio.tipo === 'NUEVO' || cambio.tipo === 'EDITAR') && cambio.archivoB64) {
                        return fetch(baseUrl + '/Partituras/' + encodeURIComponent(cambio.nombreArchivoFinal), { headers: headers })
                            .then(function(res) {
                            ...
                            .then(function(data) {
                                ...
                                return fetch(baseUrl + '/Partituras/' + encodeURIComponent(cambio.nombreArchivoFinal), {
                                    method: "PUT",
                                    headers: headers,
                            ...
                            .then(function(res) {
                                if (!res.ok) throw new Error('Fallo al subir: ' + cambio.nombreArchivoFinal);
                                // Respaldo Drive (Opcional, omitido o simplificado)
            ...
            sequencia.then(function() {
                ...
                return fetch(baseUrl + '/cantos.json', {
                    method: "PUT",
                    headers: headers,
                    body: JSON.stringify({
                ...
                if (!res.ok) throw new Error("Fallo al actualizar cantos.json");
                return res.json();
            }).then(function(data) {
                ...
                textoCarga.textContent = "Error: " + error.message;
                setTimeout(function() {
                    modalCarga.classList.add('oculto');
                    btnSincronizar.disabled = false;
                }, 5000);
